...
if __name__ == "__main__":
    list1 = [66, 77, 88, 99, 55]
    listB = [11, 22, 33]
    print(f"head(listA) = {head(list1)}")
    print(f"tail(listA) = {tail(list1)}")
    print(f"concat(listA, listB) = {concat(list1, listB)}")
    print(f"sum_of_list(listB) = {sum_of_list(listB)}")
    print(f"filter_failures = {filter_failures(list1)}")
